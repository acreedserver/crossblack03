...
function meta:FormatCheck(text)
	self.Paint = function()
		draw.SimpleText(text, "SChatText", 20, 6, Color(200, 200, 200,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER ) 
	end
...
function PANEL:Init()
	...
	self.TextB.PerformLayout = function()
		self.TextB:SetFontInternal("SChatText")
		if self.IsVis then
	...
	self.TextEntry.PerformLayout = function(self)
		self:SetFontInternal("SChatText")
...
function PANEL:Paint()		
	...
	self.Paint = function()	
		...
		draw.SimpleText(SC.Title, "SChatTitle", 4, 2, Color(255, 255, 255,self.Alpha), TEXT_ALIGN_LEFT)
	end	
	...
	self.CloseB.Paint = function()
		...
		draw.SimpleText("Settings", "SChatTitle", self.CloseB:GetWide()/2, self.CloseB:GetTall()/2, Color(255, 255, 255, self.Alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
	...
	self.EnterB.Paint = function()
		...
		if self.TeamBased then		
			draw.SimpleText("Team", "SChatText", self.EnterB:GetWide()/2, self.EnterB:GetTall()/2, col, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		else
			draw.SimpleText("Say", "SChatText", self.EnterB:GetWide()/2, self.EnterB:GetTall()/2, col, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end
	end
	...
	self.TextEntry.Paint = function()
		self.TextEntry:SetFontInternal("SChatText")
...
function PANEL:AddText(...)
	...
	for k, v in ipairs({...}) do
		...
		elseif type(v) == "table" then
			self.TextB:InsertColorChange(v.r, v.g, v.b, v.a)
		end
	end
